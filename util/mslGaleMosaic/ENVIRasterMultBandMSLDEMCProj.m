classdef ENVIRasterMultBandMSLDEMCProj < ENVIRasterMultBandEquirectProjRot0
    % ENVIRasterMultBandMSLDEMCProj
    %  ENVI style (*.img & *.hdr) multi-band image defined on a 
    %  rectangular region cropped from MSLDEMGaleMosaic_v3 data. 
    %  "C" means "Cropped".
    % 
    ...
    properties
     ...
     methods
        function obj = ENVIRasterMultBandMSLDEMCProj(basename,dirpath,varargin)
            
            obj@ENVIRasterMultBandEquirectProjRot0(...
            ...
            if ~isempty(obj.hdr)
                ...
                if isfield(obj.hdr,'msldemc_sample_offset')
                    obj.chdr.sample_offset = obj.hdr.msldemc_sample_offset;
                else
                    error('msldemc_sample_offset is not defined in the header file.');
                end
                if isfield(obj.hdr,'msldemc_line_offset')
                    obj.chdr.line_offset   = obj.hdr.msldemc_line_offset;
                else
                ...
                if ~isempty(obj.base.imgpath)
                    [obj.proj_info] = obj.get_proj_info_from_base();
                end
                
            end
        end
        
        function [proj_info_from_base] = get_proj_info_from_base(obj)
            [proj_info_from_base] = msldemc_get_proj_info_from_base(obj.base,obj);
