/* =====================================================================
 * mslortho_lazyenvireadRectUint8_mex.c
 * Read the specified rectangle region of the MSL Ortho Mosaic image data. 
 * The rectangle part of the image:
 * MSLOrthoMosaic[smpl_offset:smpl_offset+samples,
 ...
 * 
 * OUTPUTS:
 * 0  dem_img [S_demc x L_demc]   uint8_t
...
void lazyenvireadRectUint8(char *imgpath, EnviHeader hdr, 
        size_t smpl_offset, size_t line_offset,
        uint8_T **img, size_t samples, size_t lines)
{
    size_t i,j;
    long int skip_pri;
                  ...
                  int nrhs, const mxArray *prhs[])
{
     ...
     * ----------------------------------------------------------------- */
    if(nrhs!=6) {
        mexErrMsgIdAndTxt("mslortho_lazyenvireadRectUint8_mex:nrhs","Six inputs required.");
    }
    if(nlhs!=1) {
        mexErrMsgIdAndTxt("mslortho_lazyenvireadRectUint8_mex:nlhs","Five outputs required.");
    }
    /* make sure the first input argument is scalar */
    if( !mxIsChar(prhs[0]) ) {
        mexErrMsgIdAndTxt("mslortho_lazyenvireadRectUint8_mex:notChar","Input 0 needs to be a string.");
    }
    if( !mxIsStruct(prhs[1]) ) {
        mexErrMsgIdAndTxt("mslortho_lazyenvireadRectUint8_mex:notStruct","Input 1 needs to be a struct.");
    }
    if( !mxIsScalar(prhs[2]) ) {
        mexErrMsgIdAndTxt("mslortho_lazyenvireadRectUint8_mex:notScalar","Input 2 needs to be a scalar.");
    }
    if( !mxIsScalar(prhs[3]) ) {
        mexErrMsgIdAndTxt("mslortho_lazyenvireadRectUint8_mex:notScalar","Input 3 needs to be a scalar.");
    }
    if( !mxIsScalar(prhs[4]) ) {
        mexErrMsgIdAndTxt("mslortho_lazyenvireadRectUint8_mex:notScalar","Input 4 needs to be a scalar.");
    }
    if( !mxIsScalar(prhs[5]) ) {
        mexErrMsgIdAndTxt("mslortho_lazyenvireadRectUint8_mex:notScalar","Input 5 needs to be a scalar.");
    }
    ...
    if(smpl_offset_dbl>-1e-10)
        ...
        mexErrMsgIdAndTxt("mslortho_lazyenvireadRectUint8_mex:notnonnegative","Input 2 needs to be a nonnegative.");
    
    line_offset_dbl = mxGetScalar(prhs[3]);
    if(line_offset_dbl>-1e-10)
        line_offset = (mwSize) line_offset_dbl;
    else
        mexErrMsgIdAndTxt("mslortho_lazyenvireadRectUint8_mex:notnonnegative","Input 3 needs to be a nonnegative.");
    ...
    if(samples_dbl>-1e-10)
        ...
        mexErrMsgIdAndTxt("mslortho_lazyenvireadRectUint8_mex:notnonnegative","Input 4 needs to be a nonnegative.");
    
    lines_dbl = mxGetScalar(prhs[5]);
    if(lines_dbl>-1e-10)
        lines = (mwSize) lines_dbl;
    else
        mexErrMsgIdAndTxt("mslortho_lazyenvireadRectUint8_mex:notnonnegative","Input 5 needs to be a nonnegative.");
    
    plhs[0] = mxCreateNumericMatrix(samples,lines,mxUINT8_CLASS,mxREAL);
    img = set_mxUint8Matrix(plhs[0]);
     ...
     * ----------------------------------------------------------------- */
    lazyenvireadRectUint8(imgpath,hdr,(size_t) smpl_offset, 
