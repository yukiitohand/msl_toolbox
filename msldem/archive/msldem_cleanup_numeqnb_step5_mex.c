/* =====================================================================
 * msldem_cleanup_numeqnb_step5_mex.c
...
void msldem_numeqnb_clean_5thstep(char *msldem_imgpath, EnviHeader msldem_hdr,
        int8_T **msldem_numeqneighbors, int8_T **msldem_numeqnb_cl3,
        int8_T **msldem_numeqnb_cl5, int32_T wndw_size)
{
    int32_T c,l,cp;
    int32_T cc,ll,ccp;
    ...
    for(l=0;l<L_dem;l++){
        ...
        for(c=0;c<S_dem;c++){
            ...
            if(msldem_numeqnb_cl3[c][l]>0 && msldem_numeqnb_cl3[c][l]<6){
                c_min3 = (c-1>0)?(c-1):0;
                ...
                if(flg){
                    ...
                    v = v / (float) N;
                    if ((elevcl-m)*(elevcl-m) < 16*v)
                        msldem_numeqnb_cl5[c][l] = 1;
                }
            }
        }
    }
                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    */
    ...
    msldem_numeqnb_cl5 = set_mxInt8Matrix(plhs[0]);
    ...
    for(si=0;si<msldem_samples;si++){
        for(li=0;li<msldem_lines;li++)
            if(msldem_numeqnb_cl3[si][li]==-1){
                msldem_numeqnb_cl5[si][li] = -1;
            }else{
                msldem_numeqnb_cl5[si][li] = 0;
            }
    }
    // printf("sim = %d\n",S_im);
    /* -----------------------------------------------------------------
     ...
     * ----------------------------------------------------------------- */
    msldem_numeqnb_clean_5thstep(msldem_imgpath,msldem_hdr,
            msldem_numeqneighbors,msldem_numeqnb_cl3,msldem_numeqnb_cl5,wndw_size);
    
    /* free memories */
    mxFree(msldem_imgpath);
    mxFree(msldem_numeqneighbors);
    mxFree(msldem_numeqnb_cl3);
    mxFree(msldem_numeqnb_cl5);
