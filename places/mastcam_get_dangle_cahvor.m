function [dalpha,dbeta,dgamma] = mastcam_get_dangle_cahvor(cmmdl,xy_source,xy_target)
...
% pmc_source_h = cmmdl.get_p_minus_c_from_xy(xy_source);
% pmc_target_h = cmmdl.get_p_minus_c_from_xy(xy_target);
pmc_source_h = pmc_source - (cmmdl.Vdash * pmc_source) .* cmmdl.Vdash';
pmc_target_h = pmc_target - (cmmdl.Vdash * pmc_target) .* cmmdl.Vdash';
pmc_source_h = pmc_source_h ./ sqrt(sum(pmc_source_h.^2,1));
pmc_target_h = pmc_target_h ./ sqrt(sum(pmc_target_h.^2,1));
...
pmc_source_v = pmc_source - (cmmdl.Hdash * pmc_source) .* cmmdl.Hdash';
pmc_target_v = pmc_target - (cmmdl.Hdash * pmc_target) .* cmmdl.Hdash';
pmc_source_v = pmc_source_v ./ sqrt(sum(pmc_source_v.^2,1));
pmc_target_v = pmc_target_v ./ sqrt(sum(pmc_target_v.^2,1));
