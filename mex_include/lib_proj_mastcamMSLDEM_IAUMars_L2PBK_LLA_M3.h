/* =====================================================================
 * lib_proj_mastcamMSLDEM_IAUMars_L2PBK_LLA_M3.h
 ...
 * mask_obstructed_pts_in_msldemt_using_msldemc_iaumars_L2PBK_LLA_M3
 * 
 * DYU : DYnamic Update of Linked List by removing invisible points at the time it is labelled.
 *
 * ===================================================================== */
#ifndef LIB_PROJ_MASTCAMMSLDEM_IAUMARS_L2PBK_LLA_M3_H
#define LIB_PROJ_MASTCAMMSLDEM_IAUMARS_L2PBK_LLA_M3_H
...
struct MSLDEMmask_LinkedList_wpmc{
    int32_t c;
    int32_t l;
    double pmcx;
    double pmcy;
    double pmcz;
    struct MSLDEMmask_LinkedList_wpmc *next;
    struct MSLDEMmask_LinkedList_wpmc *prev;
};

void createMSLDEMmask_LLMatrix(struct MSLDEMmask_LinkedList_wpmc ****ar2d, 
        struct MSLDEMmask_LinkedList_wpmc ***ar_base, size_t N, size_t M)
{
    size_t ni;
    
    *ar2d = (struct MSLDEMmask_LinkedList_wpmc***) malloc(sizeof(struct MSLDEMmask_LinkedList_wpmc**) * N);
    *ar_base = (struct MSLDEMmask_LinkedList_wpmc**) malloc(sizeof(struct MSLDEMmask_LinkedList_wpmc*) * N * M);
    (*ar2d)[0] = &(*ar_base)[0];
    for(ni=1;ni<N;ni++){
        (*ar2d)[ni] = (*ar2d)[ni-1] + M;
    }
...
void mask_obstructed_pts_in_msldemt_using_msldemc_iaumars_L2PBK_LLADYU_M3(
        ...
        struct MSLDEMmask_LinkedList_wpmc ***ll_papmc_bin,
                        ...
                        for(xi=x_min_int;xi<x_max_int;xi++){
                            for(yi=y_min_int;yi<y_max_int;yi++){
                                ...
                                while(ll_papmc_next!=NULL){
                                    cc = ll_papmc_next->c;
                                    ll = ll_papmc_next->l;
                                    pmcx = ll_papmc_next->pmcx;
                                    pmcy = ll_papmc_next->pmcy;
                                    pmcz = ll_papmc_next->pmcz;
                                    /* evaluate line param */
