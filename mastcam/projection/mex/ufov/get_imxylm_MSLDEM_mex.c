/* =====================================================================
 * get_imxylm_MSLDEM_mex.c
 * GET coordinate values in the image coordinate of line edge of each pixel 
 * in the DEM image.
 * 
 * INPUTS:
 ...
 * 
 * OUTPUTS:
 * 0 msldemc_imxm           Doubles [(L_demc-1) x S_demc]
 * 1 msldemc_imym           Doubles [(L_demc-1) x S_demc]
 * 
 *
 *
        ...
        double **msldemc_imxm, double **msldemc_imym)
{
    ...
    for(l=0;l<msldemc_linesm1;l++){
        ...
        vpmcx = cam_V[0] * pmcx;
        for(c=0;c<msldemc_samples;c++){
            if((msldemc_imFOVmaskd[c][l]==2) && (msldemc_imFOVmaskd[c][l+1]==2)){
                dem_cl = 0.5*((double) (elevl[c] + elevlp1[c]));
                pmcz  = -dem_cl-cam_C[2];
                apmcz = cam_A[2] * pmcz;
                hpmcz = cam_H[2] * pmcz;
                vpmcz = cam_V[2] * pmcz;
                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    */
    ...
    msldemc_linesm1 = msldemc_lines-1;
    plhs[0] = mxCreateDoubleMatrix(msldemc_linesm1,msldemc_samples,mxREAL);
    msldemc_imxm = set_mxDoubleMatrix(plhs[0]);
    plhs[1] = mxCreateDoubleMatrix(msldemc_linesm1,msldemc_samples,mxREAL);
