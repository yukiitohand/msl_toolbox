/* =====================================================================
 * find_hidden_edges_mastcamMSLDEM_v6_mex.c
 ...
 * 6 S_im                  int
 * 7 L_im                  int
 * 8 msldemc_imxm           Doubles [L_demc-1 x S_demc-1]
 * 9 msldemc_imym           Doubles [L_demc-1 x S_demc-1]
 *
 ...
 * 
 * OUTPUTS:
 * 0  msldemc_inImage     [(L_demc-1) x (S_demc-1)]  Boolean
 * 
 *
 *
        ...
        int8_T **msldemcedge_inImage)
{
    ...
    double *msldemcm_northing,*msldemcm_easting;
    ...
    msldemcm_northing = (double*) malloc(sizeof(double) * (size_t) L_demcm1);
    msldemcm_easting = (double*) malloc(sizeof(double) * (size_t) S_demcm1);
    for(c=0;c<S_demcm1;c++){
        msldemcm_easting[c] = 0.5*(msldemc_easting[c]+msldemc_easting[c+1]);
    }
    for(l=0;l<L_demcm1;l++){
        msldemcm_northing[l] = 0.5*(msldemc_northing[l]+msldemc_northing[l+1]);
    }
    ...
    printf("%d\n",msldemc_samples);
    for(ll=0;ll<L_demcm1;ll++){
        for(cc=0;cc<S_demcm1;cc++){
            if(!isnan(msldemc_imxm[cc][ll])){
                xi = (int32_T) floor(msldemc_imxm[cc][ll]);
                yi = (int32_T) floor(msldemc_imym[cc][ll]);
                if(xi<0)
                    xi=0;
                else if(xi>S_imm1)
                    xi = S_imm1;
    ...
    for(c=0;c<S_demcm1;c++){
        for(l=0;l<L_demcm1;l++){
            if((msldemc_imxm[c][l]<-0.5) || (msldemc_imxm[c][l]>S_imm05) || (msldemc_imym[c][l]<-0.5) || (msldemc_imym[c][l]>L_imm05)){
                msldemcedge_inImage[c][l] = 0;
            }
        }
    }
                        ...
                        for(xi=x_min_int;xi<x_max_int;xi++){
                            ...
                            for(yi=y_min_int;yi<y_max_int;yi++){
                                for (n=0;n<bin_count_im[xi][yi];n++){
                                    cc = bin_im_c[xi][yi][n];
                                    ll = bin_im_l[xi][yi][n];
                                    if(msldemc_imxm[cc][ll] > x_min && msldemc_imxm[cc][ll] < x_max && msldemc_imym[cc][ll] > y_min && msldemc_imym[cc][ll] < y_max)
                                    {
                                        /* evaluate line param */
                                        // tppvgx = msldemc_northing[ll];
                                        // tppvgy = msldemc_easting[cc];
                                        // tppvgz = msldemc_img[cc][ll];
                                        lprm = lprm_nume/(pnx*msldemcm_northing[ll]+pny*msldemcm_easting[cc]+pnz*0.5*(msldemc_img[cc+1][ll]+msldemc_img[cc][ll+1]));
                                        if(lprm<1 && lprm>0){
                                            /* evaluate the test vector is inside the triangle. */
                                            //tppvx = msldemc_imx[cc][ll];
                                            //tppvy = msldemc_imxy[cc][ll];
                                            pipvx = msldemc_imxm[cc][ll] - ppv1x; pipvy = msldemc_imym[cc][ll] - ppv1y; 
                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    */
    ...
    /* INPUT 3/4 msldem imxy */
    msldemc_imxm = set_mxDoubleMatrix(prhs[8]);    
    msldemc_imym = set_mxDoubleMatrix(prhs[9]);
    
    
    /* OUTPUT 0/1/2 north-east-elevation */
    plhs[0] = mxCreateNumericMatrix(L_demcm1,S_demcm1,mxINT8_CLASS,mxREAL);
    msldemcedge_inImage = set_mxInt8Matrix(plhs[0]);
    
    // Initialize matrices
    for(si=0;si<S_demcm1;si++){
        for(li=0;li<L_demcm1;li++){
            msldemcedge_inImage[si][li] = (int8_T) !isnan(msldemc_imxm[si][li]);
        }
    }
    // printf("%d\n",S_im);
    /* -----------------------------------------------------------------
    ...
    find_hidden((int32_T) S_demc, (int32_T) L_demc, msldemc_img,
       ...
       msldemc_imx, msldemc_imy,msldemc_imxm,msldemc_imym,
       msldemc_imFOVmask, 
       (int32_T) S_im, (int32_T) L_im,
       msldemcedge_inImage);
