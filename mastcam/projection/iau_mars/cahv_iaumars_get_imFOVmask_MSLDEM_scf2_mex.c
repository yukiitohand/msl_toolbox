/* =====================================================================
 * cahv_iaumars_get_imFOVmask_MSLDEM_scf2_mex.c
 ...
 * 9 re                  Double Scalar equatorial radius
 * 10 rp                  Double Scalar polar radius
 * 
 * 
 * OUTPUTS:
...
void get_imFOVmask_MSLDEM(char *msldem_imgpath, EnviHeader msldem_hdr,
        ...
        CAHV_MODEL cahv_mdl,
        int8_T **msldem_imFOVmaskd, double coef_mrgn, double re, double rp)
{
    int32_T c,l;
    ...
    double Rlt[3][3]; // Local tangent rotation matrix
    double re2rp2;
    double dlatl, sin_dlatl, cos_dlatl;
    double cam_Hd_lt[3],cam_Vd_lt[3];
    double resol_H, resol_V;
    double dx,dy;
    ...
    for(l=0;l<L_dem;l++){
        ...
        dlatl = atan(re2rp2 * tan(msldem_latitude[l]));
        sin_dlatl = sin(dlatl);
        cos_dlatl = cos(dlatl);
        ...
        for(c=0;c<S_dem;c++){
            ...
            if(isnan(radius_cl)){
                ...
                if(apmc>0){
                    ...
                    Rlt[1][1] = cos_lonc;
                    Rlt[2][1] = 0;
                    Rlt[0][2] = -cos_dlatl * cos_lonc;
                    Rlt[1][2] = -cos_dlatl * sin_lonc;
                    Rlt[2][2] = -sin_dlatl;
                    ...
                    cam_Vd_lt[0] = cam_Vd[0]*Rlt[0][0] + cam_Vd[1]*Rlt[1][0] + cam_Vd[2]*Rlt[2][0];
                    cam_Vd_lt[1] = cam_Vd[0]*Rlt[0][1] + cam_Vd[1]*Rlt[1][1] + cam_Vd[2]*Rlt[2][1];
                    cam_Vd_lt[2] = cam_Vd[0]*Rlt[0][2] + cam_Vd[1]*Rlt[1][2] + cam_Vd[2]*Rlt[2][2];
                    
                    resol_H = fabs(dx*cam_Hd_lt[0]) + fabs(dy*cam_Hd_lt[1]) + fabs(resolz*cam_Hd_lt[2]);
                    resol_V = fabs(dx*cam_Vd_lt[0]) + fabs(dy*cam_Vd_lt[1]) + fabs(resolz*cam_Vd_lt[2]);

                    mrgnh = coef_mrgn*hs/apmc * resol_H;
                    mrgnv = coef_mrgn*vs/apmc * resol_V;
                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    */
    ...
    re = mxGetScalar(prhs[9]);
    rp = mxGetScalar(prhs[10]);
    ...
    get_imFOVmask_MSLDEM(mslrad_imgpath, mslrad_hdr,
        ...
        S_im, L_im, cahv_mdl,
        mslrad_imFOVmask,coef_mrgn,re,rp);
