/* =====================================================================
 * lib_proj_mastcamMSLDEM_IAUMars_L2SMP_LLP_M3_SCIMXY_STAA.h 
 ...
 * SCIMXY : SCreening in the Projected camera image coordinate (x_im,y_im) (Only supported with LLP)
 * STAA: STop if no further points than a triangle are present. (in case both c and l for triangles are searched in the ascending order)
 *
 * Two functions are included
 * mask_obstructed_pts_in_msldemt_using_msldemc_iaumars_L2SMP_LLPDYU_M3_SCIMXY_STAA
...
#ifndef LIB_PROJ_MASTCAMMSLDEM_IAUMARS_L2SMP_LLP_M3_SCIMXY_STAA_H
#define LIB_PROJ_MASTCAMMSLDEM_IAUMARS_L2SMP_LLP_M3_SCIMXY_STAA_H
...
struct MSLDEMmask_LinkedList_wimxy{
    int32_t c;
    int32_t l;
    double imx;
    double imy;
    double radius;
    struct MSLDEMmask_LinkedList_wimxy *next;
    struct MSLDEMmask_LinkedList_wimxy *prev;
};

/* with Dynamic Linked List Update */
void mask_obstructed_pts_in_msldemt_using_msldemc_iaumars_L2SMP_LLPDYU_M3_SCIMXY_STAA(
        ...
        double S_im, double L_im, CAHV_MODEL cahv_mdl)
{
    ...
    struct MSLDEMmask_LinkedList_wimxy *ll_papmc0;
    struct MSLDEMmask_LinkedList_wimxy *ll_napmc0;
...
//                         ppv3gz = ((double) -elevlp1[c]) - cam_C[2];
                    ...
                    if(isinFOVd){
                        ...
                        */
                        ...
                        ll_papmc_next = ll_papmc0;
                        ll = 2147483647;
                        while(ll_papmc_next!=NULL && ll>l){
                            imx = ll_papmc_next->imx;
                            imy = ll_papmc_next->imy;
                            if(imx>x_min && imx<x_max && imy>y_min && imy<y_max){
                                cc = ll_papmc_next->c;
                                ll = ll_papmc_next->l;
...
/* Second function without dynamic linked list update */
void mask_obstructed_pts_in_msldemt_using_msldemc_iaumars_L2SMP_LLP_M3_SCIMXY_STAA(
        char *msldem_imgpath, EnviHeader msldem_header, double mslrad_offset,
        int32_T msldemc_imxy_sample_offset, int32_T msldemc_imxy_line_offset,
        int32_T msldemc_samples, int32_T msldemc_lines,
        double *msldemc_latitude, double *msldemc_longitude, int8_T **msldemc_imFOVmask,
                                    ...
                                    if(pprm_t>0){
                                        ...
                                        if( (pprm_u>0) && (pprm_s+pprm_t+pprm_u>1) ){
                                            if((cc==cv1 && ll==lv1) || (cc==cv2 && ll==lv2) || (cc==cv3 && ll==lv3)){
                                                ...
                                                msldemt_inImage[cc][ll] = 0;
                                                /* No dynamic update of the Linked List */
                                                // if(ll_papmc_next->next!=NULL){
                                                //     ll_papmc_next->next->prev = ll_papmc_next->prev;
                                                // }
                                                // if(ll_papmc_next->prev!=NULL){
