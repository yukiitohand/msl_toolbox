/* =====================================================================
 * northeast_get_msldemtUFOVmask_ctr_L2PBK_LL0_M3_mex.c
 ...
 * 9 K_L                  double reciprocal of the length of the bin in the image line direction.
 * 10 K_S                  double reciprocal of the length of the bin in the image sample direction.
 * 11 dyu                  int8_t flag for dynamic linked list update.
 *
 * 
 * 
 * 
...
#include "cahvor.h"
#include "lib_proj_mastcamMSLDEM_northeast_L2PBK_LL0_M3.h"

#include <time.h>



void bin_msldemt_ctr_ne_L2PBK_LL0(double S_im, double L_im, CAHV_MODEL cahv_mdl,
        ...
        struct MSLDEMmask_LinkedList **ll_napmc)
{
    ...
    for(l=0;l<msldemc_lines;l++){
        ...
        x_cl = msldemc_northing[l];
        for(c=0;c<msldemc_samples;c++){
            if((msldemc_imFOVmask[c][l]>1)){
                z_cl = (double) -elevl[c];
                y_cl = msldemc_easting[c];
                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    */
    ...
    S_im = mxGetScalar(prhs[6]);
    L_im = mxGetScalar(prhs[7]);
    
    /* INPUT 9 CAHV model */
    cahv_mdl = mxGet_CAHV_MODEL(prhs[8]);
    
    /* INPUT 10/11 bin size parameters*/
    K_L = mxGetScalar(prhs[9]);
    K_S = mxGetScalar(prhs[10]);
    
    /* INPUT 12 Dynamic Linked List Update FLAG */
    dyu = (int8_t) mxGetScalar(prhs[11]);
    ...
    bin_msldemt_ctr_ne_L2PBK_LL0(S_im, L_im, cahv_mdl,
    ...
    strt_time = clock();
    if(dyu==1){
        mask_obstructed_pts_in_msldemt_using_msldemc_ne_L2PBK_LL0DYU_M3(
                msldem_imgpath, msldem_header,
            (int32_T) msldemc_imxy_sample_offset, (int32_T) msldemc_imxy_line_offset,
            (int32_T) msldemc_samples, (int32_T) msldemc_lines,
            msldemc_northing, msldemc_easting, msldemc_imFOVmask,
            (int32_T) msldemc_samples, (int32_T) msldemc_lines,
            msldemc_northing, msldemc_easting, msldemt_imUFOVmask,
            K_L,K_S,
            ll_papmc_bin, ll_napmc,
            S_im, L_im, cahv_mdl);
    } else if(dyu==0){
