...
if is_gpu
    ...
    dem_easting = gpuArray(dem_easting);
end
...
%-------------------------------------------------------------------------%
dem_imFOV_mask_cmmdl = false(L_dem,S_dem,gpu_varargin{:});
% dem_imFOV_mask_cmmdl: 
%   Solely computed from camera model. If the coordinate values is too away 
%   from the field of view, then it is considered as invalid.

dem_imFOV_mask = false(L_dem,S_dem,gpu_varargin{:});
% dem_imFOV_mask: 
%   Some safe guard is applied. Close pixels are forcefully set in the FOV
%   as such pixels are easily get too far away in the coordinate, but these
%   pixels may be necessary for calculating the intersection.
...
deml_rov0_vs = zeros(3,len_vs,precision,gpu_varargin{:});
